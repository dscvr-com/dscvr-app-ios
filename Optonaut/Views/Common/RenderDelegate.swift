...
class RenderDelegate: NSObject, SCNSceneRendererDelegate {
    ...
    fileprivate let rotationMatrixSource: RotationMatrixSource
    fileprivate let cameraOffset: Float
    
    let scene = SCNScene()
    
    fileprivate var _fov: FieldOfView
    var fov: FieldOfView {
        get {
            return _fov
        }
    ...
    internal static func getFov(_ width: CGFloat, height: CGFloat, fov: Double) -> FieldOfView {
        let xFov = Float(fov)
        let yFov = toDegrees(
            atan(
    ...
    internal static func setupCamera(_ fov: FieldOfView) -> SCNCamera {
        ...
        camera.projectionTransform = SCNMatrix4FromGLKMatrix4(projection)
...
class CubeRenderDelegate: RenderDelegate {
    ...
    fileprivate let cubeScaling: CGFloat = 10
    fileprivate let cubeFaceCount: Int
    fileprivate let autoDispose: Bool
    fileprivate var willRequestAll: Bool
    ...
    fileprivate static func getBlackTexture() -> UIImage {
    ...
    fileprivate static let BlackTexture = CubeRenderDelegate.getBlackTexture()
    ...
    fileprivate func getDist(_ a: SCNNode, b: SCNNode) -> Float {
    ...
    fileprivate func getKNNGeometric(_ center: SCNNode, k: Int) -> [SCNNode] {
        let allNodes = planes.values.map { $0.node }
        let sorted = allNodes.sorted { (n1: SCNNode, n2: SCNNode) in getDist(center, b: n1) < getDist(center, b: n2) }
        return [SCNNode](sorted.prefix(k))
    }
    
    func setTexture(_ texture: SKTexture, forIndex index: CubeImageCache.Index) {
...
//        }
    ...
    func getVisibleAndAdjacentPlaneIndicesFromRotationMatrix(_ rotation: GLKMatrix4) -> [CubeImageCache.Index] {
    ...
    fileprivate func getVisibleAndAdjacentPlanes(_ camera: SCNNode) -> Set<SCNNode> {
        let visiblePlanes: [SCNNode] = planes.values.map { $0.node }.filter { (self.scnView!.isNode($0, insideFrustumOf: camera)) }
    ...
    fileprivate func request(_ item: Item, index: CubeImageCache.Index) {
        if !item.visible {
            sync(self) {
    ...
    fileprivate func forget(_ item: Item, index: CubeImageCache.Index) {
        if item.visible {
            sync(self) {
                if item.hasTexture {
    ...
    override func renderer(_ aRenderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
    ...
    fileprivate func createPlane(position: SCNVector3, rotation: SCNVector3, subX: Float, subY: Float, subW: Float) -> SCNNode {
...
class SphereRenderDelegate: RenderDelegate {
    ...
    fileprivate func updateProjectionMatrix(_ size: CGSize, isSKTexture: Bool) {
        if size.width == size.height {
            ...
            sphereNode.geometry?.firstMaterial?.diffuse.borderColor =  UIColor.black
            
        }
        
    }
    
    fileprivate static func createSphere() -> SCNNode {
