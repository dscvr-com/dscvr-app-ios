...
class OpenGLView: UIView {
    ...
    private let queue = dispatch_queue_create("render_queue", DISPATCH_QUEUE_SERIAL)
...
//        var shaderString = try! NSString(contentsOfFile:shaderPath, encoding: NSUTF8StringEncoding)
//        if (shaderString == nil) {
//            print("Failed to set contents shader of shader file!")
//        }
        
        let str: String
        if shaderType == GLenum(GL_VERTEX_SHADER) {
            str =
            ...
            "}"
        } else if shaderType == GLenum(GL_FRAGMENT_SHADER) {
            str =
        ...
        let shaderString = str as NSString
